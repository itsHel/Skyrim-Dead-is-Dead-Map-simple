...
function customSelect(elements, optionsPicked = {}) {
    ...
    elements.forEach(function (element) {
        ...
        if (
            selectOriginal.nextElementSibling?.classList.contains(
        ...
        selectOriginal.querySelectorAll("option").forEach(function (option) {
            ...
            if (
                ...
                headerSelect =
                    '<div class="custom-select-header"><span>' +
                    option.textContent +
        ...
        if (!headerSelect) {
            ...
            items = items.replace(
                "custom-select-item ",
                "custom-select-item custom-selected",
            );
        }
        ...
        let selectWrapper = selectFullWrapper.querySelector(
            ".select-items-wrapper",
        );
        let selectHeader = selectFullWrapper.querySelector(
        ...
        let width =
            (
                parseFloat(selectWrapper.offsetWidth) -
                2 *
                    parseFloat(
                        getComputedStyle(selectWrapper.querySelector("div"))
                            .padding,
                    )
            ).toString() + "px";
        selectFullWrapper.querySelector("span:first-of-type").style.width =
            width;
            ...
            .forEach(function (item, index) {
                item.addEventListener("click", function () {
                    if (
                        ...
                        [...item.parentNode.children].forEach((el) =>
                            el.classList.remove("custom-selected"),
                        );
                        item.classList.add("custom-selected");

                        originalSelectChangedByThis = true;
                        selectOriginal.nextElementSibling.querySelector(
                            "span",
                        ).textContent = this.textContent;

                        [...selectOriginal.children].forEach(
                            function (el, _index) {
                                if (index == _index) {
        ...
        selectHeader.addEventListener("click", function () {
            if (!selectWrapper.innerHTML) return;

            if (
                options.dontClose ||
                (options.dontCloseCallback && options.dontCloseCallback())
            ) {
                selectWrapper
                    .querySelector(".custom-selected")
                    ?.classList.remove("custom-selected");
            } else {
                selectWrapper
                    .querySelectorAll(".custom-selected-multi")
                    .forEach((item) =>
                        item.classList.remove("custom-selected-multi"),
                    );
            }
            ...
            if (selectWrapper.style.maxHeight == "0px") {
                ...
                if (
                    selectHeader.getBoundingClientRect().y +
        ...
        window.addEventListener("click", function (e) {
            if (
                ...
                (options.dontCloseCallback &&
                    options.dontCloseCallback() &&
                    e.target.classList.contains("custom-select-item"))
            )
                return;
            ...
            if (
                !e.target ||
                !e.target.parentNode ||
                (e.target.className != "custom-select-header" &&
                    e.target.parentNode.className != "custom-select-header")
            ) {
                selectWrapper.style.maxHeight = "0px";
                selectFullWrapper.classList.remove("select-open");
            }
        ...
        selectOriginal.addEventListener("change", function () {
            if (!originalSelectChangedByThis) {
                ...
                if (
                    selectWrapper.querySelector(".custom-selected")
                    ...
                    [...selectWrapper.children].forEach(function (child) {
                        if (child.textContent == selected) {
                            ...
                            [...child.parentNode.children].forEach((el) =>
                                el.classList.remove("custom-selected"),
                            );
        ...
        if (options.scrollOnKey) {
            ...
            customSelect.addEventListener("keydown", function (e) {
                ...
                let scrolled = false;
                customSelect
                    .querySelectorAll(".select-items-wrapper div:not(:first)")
                    .each(function (el) {
                        if (
                            !scrolled &&
                            el.textContent[0].toLowerCase() == e.key
                        ) {
                            scrolled = true;
                            ...
                            customSelect
                                .querySelector(".select-items-wrapper")
